...
class UpdateServer extends FormRequest
{
    ...
    public function rules()
    {
        return [
            ...
            // 'slug' => ['required', 'string', 'alpha_dash', 'min:3', 'max:24', Rule::unique('servers')->ignore(request()->id)],
            'description' => ['nullable'],
            'host' => ['required', 'min:3', 'max:24', Rule::unique('servers')->ignore(request()->id)],
            'port' => ['required', 'integer', 'gte:0'],
            'version_id' => ['required', 'integer'],
            'type_id' => ['required', 'integer'],
            'country_id' => ['required', 'integer'],
            'link_website' => ['nullable', 'active_url'],
            'webhook_discord' => ['nullable', 'active_url'],
